...
if __name__ == '__main__':
    ...
    def end_callback(event):
        print('Haha! I know where this ends!!!')
    ...
    def pos_callback(event, player):
        if echo_position:
            m=MediaStats()
            player.get_media().get_stats(m)
            print 'input_bitrate:',m.input_bitrate ,'send_bitrate:',m.send_bitrate,'read_bytes',m.read_bytes,'\n'
            print >> outfile, ','.join([str(m.read_bytes),str(m.input_bitrate),str(m.demux_read_bytes),str(m.demux_bitrate),str(m.sent_bytes),str(m.send_bitrate)])
