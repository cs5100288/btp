...
public class Shell {
	...
	public OutputHandler mOutputHandler;
	...
	private void readOutput() throws IOException, InterruptedException {
		...
		while (true) {
			...
			if(mOutputHandler!=null)
				mOutputHandler.handleOutput(line);
			// terminate on EOF
			if (line == null)
				break;
			...
			if (command == null) {
//				Log.d("RootTools.Shell", "Command null, read: "+read+" commands.size: "+commands.size());
...
//			Log.d("RootTools.Shell", "Token pos: "+pos);
			if (pos > 0)
			...
			if (pos >= 0) {
				...
				if (fields.length >= 2 && fields[1] != null)
				{
                    ...
                    Log.d("RootTools.Shell", "id: "+id+", read: "+read);
					if (id == read) {
