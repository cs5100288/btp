...
class ols:
    ...
    def summary(self):
        ...
        # op+=ing output to screen
        op+= '\n=============================================================================='
        op+="\n"
        op+= "Dependent Variable: " + self.y_varnm
        op+="\n"
        op+= "Method: Least Squares"
        op+="\n"
        op+= "Date: "+ time.strftime("%a, %d %b %Y",t)
        op+="\n"
        op+= "Time: "+ time.strftime("%H:%M:%S",t)
        op+="\n"
        op+= '# obs:               %5.0f' % self.nobs
        op+="\n"
        op+= '# variables:     %5.0f' % self.ncoef
        op+="\n"
        op+= '=============================================================================='
        op+="\n"
        op+= 'variable     coefficient     std. Error      t-statistic     prob.'
        op+="\n"
        op+= '=============================================================================='
        op+="\n"
        for i in range(len(self.x_varnm)):
            op+= '''% -5s          % -5.6f     % -5.6f     % -5.6f     % -5.6f''' % tuple([self.x_varnm[i],self.b[i],self.se[i],self.t[i],self.p[i]])
            op+="\n"
        op+= '=============================================================================='
        op+="\n"
        op+= 'Models stats                         Residual stats'
        op+="\n"
        op+= '=============================================================================='
        op+="\n"
        op+= 'R-squared            % -5.6f         Durbin-Watson stat  % -5.6f' % tuple([self.R2, self.dw()])
        op+="\n"
        #op+= 'Adjusted R-squared   % -5.6f         Omnibus stat        % -5.6f' % tuple([self.R2adj, omni])
        #op+= 'F-statistic          % -5.6f         Prob(Omnibus stat)  % -5.6f' % tuple([self.F, omnipv])
        op+= 'Prob (F-statistic)   % -5.6f			JB stat             % -5.6f' % tuple([self.Fpv, JB])
        op+="\n"
        op+= 'Log likelihood       % -5.6f			Prob(JB)            % -5.6f' % tuple([ll, JBpv])
        op+="\n"
        op+= 'AIC criterion        % -5.6f         Skew                % -5.6f' % tuple([aic, skew])
        op+="\n"
        op+= 'BIC criterion        % -5.6f         Kurtosis            % -5.6f' % tuple([bic, kurtosis])
        op+="\n"
        op+= '=============================================================================='
        op+="\n"
        return op
